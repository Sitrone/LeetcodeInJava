...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    // FILL IN YOUR SOLUTION TO PART II HERE.
      ...
      SibTreeNode insert = new SibTreeNode(this.myTree, item);
      insert.parent = this;
      ...
      if(c <= 1)
      {
          insert.nextSibling = this.firstChild;
          this.firstChild = insert;
          this.myTree.size++;
          return;
      }
      else if(c > 1 && c < this.children() + 1)
      {
          while(cur != null && --c > 1)
          {
              cur = cur.nextSibling;
          }
          insert.nextSibling= cur.nextSibling;
      }
      else
      {
  ...
  public void removeLeaf() throws InvalidNodeException {
    // FILL IN YOUR SOLUTION TO PART III HERE.
      // (root,  notroot(children == 0, children != 0(one children, children > 1)))
      if(!isValidNode())
      {
          throw new InvalidNodeException();
      }
      ...
      if(this.parent != null)
      {
          ...
          if(node.equals(this))  // first child
          {
              if(node.nextSibling != null)
              {
                  this.parent.firstChild = node.nextSibling;
              }
              else
              {
